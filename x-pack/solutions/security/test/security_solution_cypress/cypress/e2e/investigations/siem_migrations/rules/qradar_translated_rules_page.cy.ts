...
describe(
  'QRadar Rule Migrations - Translated Rules Page',
  ...
  () => {
    ...
    it('should be able to see the result of the completed QRadar migration', () => {
      cy.get(TRANSLATED_RULES_RESULT_TABLE.ROWS).should('have.length.at.least', 1);
      cy.get(TRANSLATED_RULES_RESULT_TABLE.TABLE).should('be.visible');
    });

    it('should be able to edit a QRadar rule with partial translation', () => {
      cy.get(TRANSLATED_RULES_RESULT_TABLE.TABLE).should('be.visible');
      ...
      cy.get(TRANSLATED_RULE_RESULT_BADGE).should('exist');
    });

    it('should be able to reprocess a failed QRadar Rule', () => {
      cy.intercept({
        url: '**/start',
      ...
      cy.get(RULE_MIGRATION_PROGRESS_BAR_TEXT).should('be.visible');
    });
  }
);
